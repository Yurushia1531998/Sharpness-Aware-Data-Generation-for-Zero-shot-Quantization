...
class DifferentiableOptimizer(_abc.ABC):
    ...
    def step(
        ...
        **kwargs
        ...
        print("Group to list:")
        print(self._group_to_param_list)
        '''
...
class DifferentiableAdam(DifferentiableOptimizer):
    ...
    def _update(self, grouped_grads: _GroupedGradsType, **kwargs) -> None:
        ...
        zipped = zip(self.param_groups, grouped_grads)
        '''
        print("Group params ",len(self.param_groups))
        print(self.param_groups)
 
        print("grouped_grads ",len(grouped_grads))
        print(grouped_grads)
        '''
        for group_idx, (group, grads) in enumerate(zipped):
            '''
            print("Index ",group_idx)
            print("group ",group)
            print("grads ")
            print(grads)
            '''
            ...
            for p_idx, (p, g) in enumerate(zip(group['params'], grads)):
                '''
                print(" Sub group index ",p_idx)
                print("subgroup p ",p.size())
                print("subgrads ")
                print(g)
                '''
                #if g is not None:
                    #print("max and min g ",_torch.max(g),_torch.min(g),_torch.max(_torch.abs(g)),_torch.min(_torch.abs(g)))
                ...
                #print("original exp_avg_sq ",_torch.max(exp_avg_sq),_torch.min(exp_avg_sq),exp_avg_sq.size(),beta2)
                if amsgrad:
                ...
                if amsgrad:
                    ...
                    '''
                    print("value of exp_avg_sq:",exp_avg_sq)
                    print(_torch.isnan(exp_avg_sq).any())
                    print(_torch.min(_torch.abs(exp_avg_sq)),_torch.max(_torch.abs(exp_avg_sq))) 
                    print(_torch.min(exp_avg_sq),_torch.max(exp_avg_sq)) 
                    print(exp_avg_sq.sqrt())
                    '''
