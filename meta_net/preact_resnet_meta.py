...
from meta_net.meta_layers import *
...
class BasicBlockMeta(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion*planes:
            print('Creating self.downsample ')
...
class BottleneckMeta(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv1 = MetaConv2d(in_planes, planes, kernel_size=1, bias=False)
        self.bn1 = etaBatchNorm2d(in_planes)
        self.conv2 = MetaConv2d(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = MetaBatchNorm2d(planes)
        self.conv3 = MetaConv2d(planes, self.expansion*planes, kernel_size=1, bias=False)
...
def preact_resnet_meta18(): return PreActResNetMeta(PreActBlockMeta, [2,2,2,2],num_classes=1000)
def resnet_meta18(): return ResNetMeta(BasicBlockMeta, [2,2,2,2])
